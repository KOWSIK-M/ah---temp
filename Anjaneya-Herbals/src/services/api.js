...
// In development, prefer an explicit backend origin so cookies set by
// the backend (HttpOnly refresh/access cookies) are stored for that
...
export const productsApi = {
  getAll: async (params = {}) => {
    ...
    if (params.categoryId || params.category)
      queryParams.append("categoryId", params.categoryId || params.category);
...
export const adminApi = {
  ...
  getCustomerStats: async (params = {}) => {
    ...
    const response = await apiFetch(
      `/admin/customers/stats?${queryParams.toString()}`,
    );
...
export const reviewsApi = {
  ...
  getReviews: async (productId, params = {}) => {
    ...
    const response = await fetch(
      `${API_BASE_URL}/products/${productId}/reviews${qs ? "?" + qs : ""}`,
    );
...
export const couponApi = {
  ...
  update: async (id, data) => {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
      throw new Error(body.message || "Failed to update coupon");
    return body;
  },
...
export const chatApi = {
  ...
  semanticSearch: async (query) => {
    const response = await fetch(
      `${API_BASE_URL}/search/semantic?q=${encodeURIComponent(query)}`,
    );
